...
/**
 * 基本上是 MyBatisGenerator 的原版，只是修改了writeGeneratedXmlFile方法，用来覆盖xmlmapper,原版是合并
...
public class MybatisGenerator {
    ...
    private void writeGeneratedJavaFile(GeneratedJavaFile gjf, ProgressCallback callback)
        ...
        String type =  gjf.getCompilationUnit().getType().getFullyQualifiedName();
        //dao
        if(gjf.getCompilationUnit().isJavaInterface() && type.endsWith("Mapper") && !MybatisGeneratorConfig.getBoolean("generator.dao")){
            return;
        }
        //model
        if(!type.endsWith("Dto") && !type.endsWith("ServiceImpl") && !gjf.getCompilationUnit().isJavaInterface() && !MybatisGeneratorConfig.getBoolean("generator.model")){
            return;
        }
        //dto
        if(!gjf.getCompilationUnit().isJavaInterface()){
            if(type.endsWith("ConditionDto")){
                if(!MybatisGeneratorConfig.getBoolean("generator.service.searchDsfMethod")){
                    return;
                }

            }else if(type.endsWith("FormDto")){
                ...
                if( !MybatisGeneratorConfig.getBoolean("generator.dto")){
                    return;
                }
            }
        ...
        // service
        if(gjf.getCompilationUnit().isJavaInterface() && type.endsWith("Service") && !MybatisGeneratorConfig.getBoolean("generator.service")){
            return;
        }
        // serviceImpl
        if(!gjf.getCompilationUnit().isJavaInterface() && type.endsWith("ServiceImpl") && !MybatisGeneratorConfig.getBoolean("generator.serviceImpl")){
            return;
        }
        File targetFile;
        String source;
    ...
    private void writeGeneratedXmlFile(GeneratedXmlFile gxf, ProgressCallback callback)
            ...
            if(!MybatisGeneratorConfig.getBoolean("generator.xmlMapper"))
            return;

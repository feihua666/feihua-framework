...
public class EmailUtils {
	...
	public static Session createSession(String host, final String username, final String password,boolean validate, boolean isSSL,String mailServerPort) {
		Properties prop = new Properties();
		prop.setProperty("mail.smtp.host", host);
		prop.put("mail.smtp.port", mailServerPort);
		prop.setProperty("mail.sender.username",username);
		prop.setProperty("mail.sender.password", password);
		prop.setProperty("mail.smtp.auth",validate ? "true" : "false");
		if (isSSL) {
			prop.put("mail.smtp.starttls.enable", "true");
			prop.put("mail.smtp.socketFactory.fallback", "false");
			prop.put("mail.smtp.socketFactory.port", mailServerPort);
		}
		// 创建验证器

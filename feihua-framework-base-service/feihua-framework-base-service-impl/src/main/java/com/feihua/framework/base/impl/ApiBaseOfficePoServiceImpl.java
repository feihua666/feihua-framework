...
public class ApiBaseOfficePoServiceImpl extends ApiBaseTreeServiceImpl<BaseOfficePo, BaseOfficeDto, String> implements ApiBaseOfficePoService {
    ...
    public BaseOfficeDto selectOfficeByRoleId(String roleId) {
        ...
        if (rolePo != null && StringUtils.isNotEmpty(rolePo.getDataOfficeId())) {
    ...
    public PageResultDto<BaseOfficeDto> searchOfficesDsf(SearchOfficesConditionDto conditionDto, PageAndOrderbyParamDto pageAndOrderbyParamDto) {
        ...
        BaseOfficeDataScopeDefineDto officeDataScopeDefineDto = apiBaseDataScopeService.selectDataScopeDefineByUserId(conditionDto.getCurrentUserId(),conditionDto.getCurrentRoleId());
        // 如果未设置数据范围定义，没有数据
        ...
        if(DictEnum.OfficeDataScope.roleoffice.name().equals(officeDataScopeDefineDto.getType())){
            BaseOfficeDto officeDto = this.selectOfficeByRoleId(conditionDto.getCurrentRoleId());
            // 如果机构不存在，直接返回空
            if(officeDto == null){
                return new PageResultDto(null, null);
            }
